...
def won(player: Player, board: Board) -> bool:
    return check_rows(board, player) or check_columns(board, player) or check_diagonal(board, player)
...
def play_game(board_size: int = None):
    ...
    while True:
        ...
        err = update_board(board, current_player, coordinates)
        if err:
            print(err)
            break
        show_board(board) 
